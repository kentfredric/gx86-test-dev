...
src_prepare() {
	...
	# stop struggling with '-g' and '-O*' in CFLAGS/CXXFLAGS
	sed -i -e '/^CFLAGS=.*sed/d;/^CXXFLAGS=.*sed/d;/use_opt=/s/yes/no/' configure ||
		die "cannot stop struggling with CFLAGS/CXXFLAGS"
